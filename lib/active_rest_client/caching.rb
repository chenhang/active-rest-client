    ...
    module ClassMethods
      ...
      def read_cached_response(request)
        if cache_store && perform_caching
          ...
          ActiveRestClient::Logger.debug "  \033[1;4;32m#{ActiveRestClient::NAME}\033[0m #{key}--#{request} - Trying to read from cache"
      ...
      def write_cached_response(request, response, result)
        ...
        if cache_store && (headers[:etag] || headers[:expires])
          key = "#{request.class_name}:#{request.get_params[:id]}#{request.get_params[:page]}"
          ActiveRestClient::Logger.debug "  \033[1;4;32m#{ActiveRestClient::NAME}\033[0m #{key}--#{request} - Writing to cache"
